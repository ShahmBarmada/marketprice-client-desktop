...
const { data: users, status, refresh } = await useLazyAsyncData<IUser[]>('users', () => $fetch(`${api.url}/users/admin`), {
  default: () => [],
})
...
async function handleChange(id: number, value: boolean) {
  await $fetch(`${api.url}/users/${id}`, { method: 'PUT', body: { activated: value } })
}
</script>
  ...
  <div v-if="users" class="space-y-4">
    ...
    >
      ...
      <template #activated-data="{ row }">
        <UCheckbox v-model="row.activated" @change="handleChange(row.id, row.activated)" />
      </template>
      ...
      <template #actions-data="{ row }">
        <UTooltip text="تعديل">
          <UsersEdit :id="row.id" @close="refresh" />
        </UTooltip>
      </template>
    </UTable>
  </div>
