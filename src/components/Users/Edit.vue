...
const initialData = await $fetch<IUser>(`${api.url}/users/${props.id}`)
...
async function submitForm(event: FormSubmitEvent<Schema>) {
  const res = await $fetch<any>(`${api.url}/users/${props.id}`, { method: 'PUT', body: event.data }).catch(err => err.data)
  if (res.statusCode) {
        ...
        <template #header>
          <div class="flex items-center justify-between">
            <h3 class="text-base font-semibold leading-6 text-gray-900 dark:text-white">
              {{ `تعديل مستخدم: ${initialData.label}` }}
            </h3>
            <UButton
              color="red"
              variant="solid"
        ...
        <div>
          ...
          >
            ...
            <div class="flex flex-row flex-nowrap justify-between">
              ...
              <UFormGroup label="تفعيل ؟" name="activated" class="place-self-center flex gap-4 items-center mx-auto">
