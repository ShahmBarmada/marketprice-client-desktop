...
const props = defineProps<{ area: number }>()

const api = useAPI()
const msgErr = ref('')
const msgSucc = ref('')

const initialData = await $fetch<IArea>(`${api.url}/areas/${props.area}`)

function initAreaModel() {
  let lv2: IArea, lv3: IArea
  switch (initialData.level) {
    case 2:
      formData.lv1 = areaOpt.value.filter(
        (item) => item.id === initialData.parent
      )[0].id
      break
    case 3:
...
const formSchema = yup.object({
  ...
  lv1: yup.number().when('level', {
    is: (val: number) => val > 1,
    then: (schema) =>
      schema.positive('هذا الحقل مطلوب').required('هذا الحقل مطلوب'),
  }),
...
async function submitForm(event: FormSubmitEvent<Schema>) {
  const res = await $fetch<any>(`${api.url}/areas/${props.area}`, {
    method: 'PUT',
    body: event.data,
  }).catch((err) => err.data)
  if (res.statusCode) {
  ...
  <div v-if="initialData">
    ...
    >
      ...
      <UFormGroup label="اجور الخدمة" name="fee">
        <UInput v-model="formData.fee" type="number" min="0" step=".01" />
      </UFormGroup>
